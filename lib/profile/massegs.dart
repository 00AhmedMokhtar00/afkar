...
class _MassegsState extends State<Massegs> {
  ...
  Widget customCard(MessageModel messageModel){
    ...
    return GestureDetector(
      onTap: (){
        Navigator.push(context, MaterialPageRoute(builder: (context)=>Chat(id,messageModel.userId,messageModel.userChatId,messageModel.name)));
      },
          child: Container(
      ...
      Container(
        ...
        child: GestureDetector(
          onTap: (){
            Navigator.push(context, MaterialPageRoute(builder: (context)=>Profile(id , messageModel.userId)));
          },
                  child: ClipRRect(
      ...
      Column(
        ...
        children:<Widget>[
          Text(messageModel.name ,style: TextStyle(color: Colors.black54 , fontSize:14,fontWeight: FontWeight.w500),),
          Container(
            width: 200,
            child: Text(messageModel.lastMessage,overflow: TextOverflow.ellipsis,textAlign: TextAlign.start ,style: TextStyle(color: Colors.black54 , fontSize:12),)),
        ]
      )
                      ],),
                    ),
                    Column(
                      children: <Widget>[
                        messageModel.unreadMessagesCount > 0 ? Container(
                          alignment: Alignment.center,
                          width: messageModel.unreadMessagesCount>999?35:25,
                          height: 25,
                          ...
                          child: Text(messageModel.unreadMessagesCount.toString() , style: TextStyle(color: Colors.white , fontSize: 12),),
                        ):Container(),
                      ],
                    )
                  ]
  ...
  Future<List<MessageModel>>getData() async{
    ...
    try{
      var url = "https://afkarestithmar.com/chat2/servicesapis.php?ApiType=getUsers&userID=$id";
      http.Response request = await http.get(url);
        ...
        if(decodedData[0] == "success") {
          ...
          for(int i = 0 ; i < messageModel.length ; i++){
            messageModel[i].lastMessage = await getLastMessage(id, messageModel[i].userId);
          }
          return messageModel;
        }
        return null;
