...
class UpdateProfile extends StatefulWidget {
  final String userId,imgurl , name  , phone ,about,email ;
  UpdateProfile(this.userId,this.imgurl,this.name,this.phone,this.about ,this.email);
  @override
  _UpdateProfileState createState() => _UpdateProfileState(this.userId,this.imgurl,this.name,this.phone,this.about,this.email);
}

class _UpdateProfileState extends State<UpdateProfile> {

  final _formKey=GlobalKey<FormState>();
  String userId,imgurl , name , phone ,about,email;
  _UpdateProfileState(this.userId,this.imgurl,this.name,this.phone,this.about,this.email);
   String messageGren = "";
   String messageRed = "";
  ...
  void initState() {
    super.initState();
    _controllerAbout = new TextEditingController(text: '$about');
  }
            ...
            children: <Widget>[
              Container(
                ...
                child: Column(
                  ...
                  errorWidget: (context, url, error) => Image(image: AssetImage("images/profile2.png")),
                    ...
                    Text("$name", style:TextStyle(fontWeight: FontWeight.w400 , fontSize: 15, color: Colors.black54),),
              ...
              Column(
                children:<Widget>[
                  ...
                  child: customCard(Icons.person , "الاسم كاملاً", "$name")),
                  GestureDetector(
                    onTap: (){
                      _asyncChangesDialog(context, "البريد الالكتروني", "email");
                    },
                    child: customCard(Icons.message, "البريد الالكتروني", "$email")),
                  GestureDetector(
                    onTap: (){
                      _asyncChangesDialog(context, "رقم الهاتف", "phone");
                    },
                  child: customCard(Icons.phone, "رقم الجوال", "$phone")),
                  GestureDetector(
                  onTap: (){
                    _asyncPasswordDialog(context);
                  },
                  child: customCard(Icons.confirmation_number, "تحديث كلمة المرور", ""))
                ]
              ),
                            ...
                            Container(
                              ...
                              child: ButtonTheme(
                                ...
                                child: RaisedButton(
                                  ...
                                  onPressed: () {
                                    setState(() {
                                      this.about = _controllerAbout.text;
                                    });
                                    setData(userId, name, phone, imgurl, about);
                                  },
    ...
    return Card(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      SizedBox(width:15),
                       Icon(Icons.edit , size:22 , color:Theme.of(context).primaryColor),
                          ]
                        )
          ...
          // ),
            ...
            onTap: ()async{
                  await ImagePicker.pickImage(source: ImageSource.gallery).then((file){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=> UploadImageProfile(image:file, pro:true) ));
                  });
                },       
          ),
           ListTile(
            ...
            onTap: ()  {
              updateProfileImage(context, "5ec6c63caf6b8_profile.png").then((c){
                Navigator.pop(context);
              Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>UpdateProfile(appState.getid, "https://afkarestithmar.com/userfiles/5ec6c63caf6b8_profile.png", appState.getname, appState.getphone, appState.getabout, appState.getemail)));
              });
            },          
          ),
            ],
...
Future<String> _asyncChangesDialog(BuildContext context , String type , String object ) async {
    ...
    AlertDialog(
        ...
        content: Form(
          key: _formKey,
          child: TextFormField(
              style: TextStyle(fontSize: fontsmll*0.9 , fontFamily:'cairo', ),
            decoration: InputDecoration(
              labelStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo', ),
              hintStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo' ,color: Colors.black26),
        ...
        content: Form(
          ...
          child: Wrap(
            children: <Widget>[
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo', ),
                  hintStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo' ,color: Colors.black26),
                    labelText: 'كلمة المرور الجديدة', hintText: '' , ),
                    controller: _controller,
                    validator: (input) =>
              input.isEmpty ? 'برجاء ملئ هذا الحقل' : null,
              ),
            ...
            decoration: InputDecoration(
              labelStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo', ),
              hintStyle: TextStyle(fontSize: fontsmll*0.9  , fontFamily:'cairo' ,color: Colors.black26),
                labelText: 'اعد كتابة كلمة المرور', hintText: '' , ),
        ...
        actions: <Widget>[
          FlatButton(
           ...
           FlatButton(
            ...
            style: TextStyle(fontSize: fontsmll*0.85 , fontWeight: FontWeight.normal,color:Colors.black )),
            onPressed: ()  {
              if(_formKey.currentState.validate() && _controller.text == _controller2.text){
              setState(() {
               updatePass(userId, _controller.text);
              });
              Navigator.pop(context);
              }
            },
...
setData(String id , String name , String phone , String img , String about) async{
    setState(() {
    messageRed = "";
  messageGren = "";
  });
    try{var url = "https://afkarestithmar.com/api/api.php?type=updateAll&id=$id&name=$name&Mobile=$phone&img=$img&about=$about&Email=$email";
...
updatePass(String id , String password) async{
  ...
  messageGren = "";
    ...
    try{var url = "https://afkarestithmar.com/api/api.php?type=updatepass&pass=$password&id=$id";
        var request = await http.get(url);
        print(request.body);
        var data = jsonDecode(request.body);
        if("${data['success']}"== "1"){
